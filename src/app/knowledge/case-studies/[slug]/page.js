...
export default function CaseStudyDetail({ params }) {
  ...
  const studySections = parseContent(study.content);

  return (
    <div className={styles.main}>
      <section className={styles.header}>
        <div className="container">
          <Link href="/knowledge/case-studies" className={styles.backLink}>← Back to Case Studies</Link>
          <h1 className={styles.pageTitle}>{study.title}</h1>
          <p className={styles.pageSubtitle}>{study.description}</p>
        </div>
      </section>
